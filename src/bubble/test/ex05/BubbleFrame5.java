...
import bubble.test.ex05.Player5;

import javax.swing.*;
import java.awt.event.KeyAdapter;
import java.awt.event.KeyEvent;

public class BubbleFrame5 extends JFrame {
    ...
    private void initObject() {
        ...
        player5 = new Player5();
        add(player5);// player를 패널(JLabel) 덮붙이기
            ...
            addKeyListener(new KeyAdapter() {
               ...
               @Override
                public void keyPressed(KeyEvent e) {
                   ...
                   switch(e.getKeyCode()){
                       ...
                       case KeyEvent.VK_RIGHT:
                           if(!player5.isRight()){ //isRight()가 true가 아니라면
                                player5.right(); // Right는 한번만 실행된다. -> 메소드를 계속 호출하면 과부하가 심해지기때문에
                           }

                           break;
                       case KeyEvent.VK_UP:
                           //점프 상태가 아닐 때(up, down)만 up을 할 수 있다.
                           if(!player5.isUp() && !player5.isDown()){
                           player5.up();
                           }
                           break;
                   }
                ...
                public void keyReleased(KeyEvent e) {
                    switch (e.getKeyCode()){
                        case KeyEvent.VK_LEFT:
                            player5.setLeft(false); // 키보드를 땟을 때 -> 움직이지 않기
                            break;
                        case KeyEvent.VK_RIGHT:
                            player5.setRight(false); // 키보드를 땟을 때 -> 움직이지 않기
                            break;
                    }
                }
            });
