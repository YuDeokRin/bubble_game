...
@Getter @Setter
public class BubbleFrame18 extends JFrame {

    private BubbleFrame18 mContext = this;
    private JLabel backgroundMap;
    private Player18 player18;
    private Enemy enemy;  // 적을 많이 만들 때 -> List<Enemy> enemy

    public BubbleFrame18() {
        initObject();
        initSetting();
        initListener();
        setVisible(true);
    ...
    private void initObject() {
        ...
        player18 = new Player18(mContext);
        add(player18);
            ...
            addKeyListener(new KeyAdapter() {
               ...
               @Override
                public void keyPressed(KeyEvent e) {
                   ...
                   switch(e.getKeyCode()){
                       case KeyEvent.VK_LEFT:
                           if(!player18.isLeft() && !player18.isLeftWallCrash()){
                               player18.left();
                           }
                           break;
                       case KeyEvent.VK_RIGHT:
                           if(!player18.isRight() && !player18.isRightWallCrash()){ //isRight()가 true가 아니라면
                                player18.right(); // Right는 한번만 실행된다. -> 메소드를 계속 호출하면 과부하가 심해지기때문에
                           }

                           break;
                       case KeyEvent.VK_UP:
                           //점프 상태가 아닐 때(up, down)만 up을 할 수 있다.
                           if(!player18.isUp() && !player18.isDown()){
                           player18.up();
                           }
                           break;
                ...
                public void keyReleased(KeyEvent e) {
                    switch (e.getKeyCode()){
                        case KeyEvent.VK_LEFT:
                            player18.setLeft(false); // 키보드를 땟을 때 -> 움직이지 않기
                            break;
                        case KeyEvent.VK_RIGHT:
                            player18.setRight(false); // 키보드를 땟을 때 -> 움직이지 않기
                            break;
                    }
                }
            });
